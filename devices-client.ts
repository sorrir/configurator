...
function initDevices(devicesRaw) {
    ...
    for(const dev of devicesRaw["devices"]) {
        const devTransformed = {
            ...
            // todo: elems -> enum
            protocols: dev.protocols.map(prot => conn_protocol[prot]),
            location: "unknown",
            units: [],
        }
  ...
  const devExt = {
    deviceName: "ext",
    architecture: "n/a",
    protocols: [ conn_protocol.REST, conn_protocol.MQTT, conn_protocol.MQTT_EXTERNAL, conn_protocol.BLE, conn_protocol.TOM ],
    location: "unknown",
    units: [],
  }
  devices.push(devExt);
